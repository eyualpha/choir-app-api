...
describe("Assignment routes", () => {
  ...
  it("returns empty groups when no assignment document exists", async () => {
    const member = await createUser({ email: "assign-empty@test.com" });
    ...
    expect(res.body.assignment.leadSingers).toEqual([]);
    expect(res.body.assignment.backupSingers).toEqual([]);
    expect(res.body.assignment.prayerTeam).toEqual([]);
  });
